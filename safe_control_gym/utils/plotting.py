...
def smooth_runs(xy_list, window=10):
    ...
    smoothed_list = [window_func(np.asarray(x), np.asarray(y), window, np.mean) for x, y in xy_list]
...
def plot_from_experiments(
    ...
    for i, name in enumerate(processed_stats.keys()):
        ...
        plt.fill_between(
            x, y_mean + num_std * y_std, y_mean - num_std * y_std, alpha=0.3, color=color
        )
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
