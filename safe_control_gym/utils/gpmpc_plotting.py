...
def get_constraint_violations(test_runs, train_runs):
    ...
    for epoch in range(num_epochs):
        ...
        for test_episode in range(num_test_episodes_per_epoch):
            ...
            violations = 0
            max_violations = np.zeros(
                test_runs[epoch][test_episode]["info"][0]["constraint_values"].shape
            )
            n = len(test_runs[epoch][test_episode]["info"])  # number of samples in episode
            for i in range(n):
                violations += int(
                    np.any(test_runs[epoch][test_episode]["info"][i]["constraint_values"] > 0)
                )
                max_violations = np.maximum(
                    max_violations, test_runs[epoch][test_episode]["info"][i]["constraint_values"]
                )
...
def plot_constraint_violation(viol_samp, viols, dir):
    ...
    fname = os.path.join(dir, stem + ".csv")
    np.savetxt(fname, data, delimiter=",", header="Train Steps (s),Mean,Max,Min")
...
def get_quad_average_rmse_error_xyz(runs, ref):
    ...
    for epoch in range(num_epochs):
        for episode in range(num_episodes):
            mse, rmse = compute_state_rmse(
                runs[epoch][episode]["obs"][:, [0, 2, 4]] - ref[:, [0, 2, 4]]
            )
            costs[epoch, episode] = rmse
...
def make_plots(test_runs, train_runs, dir):
    ...
    nsamp, viol, mean_viol, maximums = get_constraint_violations(test_runs, train_runs)
    plot_constraint_violation(num_points_per_epoch, viol, fig_dir)

    # Plot Learning Curves
    avg_rmse_error = get_average_rmse_error(test_runs)
    plot_learning_curve(
        avg_rmse_error, num_points_per_epoch, "avg_rmse_cost_learning_curve", fig_dir
    )
        ...
        for episode in range(num_episodes):
            if test_runs[epoch][episode]["obs"].shape[0] != ref_length:
                ...
                for i in range(ref_length - test_runs[epoch][episode]["obs"].shape[0]):
                    test_runs[epoch][episode]["obs"] = np.vstack(
                        (test_runs[epoch][episode]["obs"], last_state)
                    )
    return test_runs
