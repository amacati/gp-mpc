...
class EpochExperiment(BaseExperiment):
    ...
    def __init__(self, env, ctrl, train_env=None, safety_filter=None, verbose: bool = False):
        super().__init__(
            env=env, ctrl=ctrl, train_env=train_env, safety_filter=safety_filter, verbose=verbose
        )
    ...
    def launch_training(self, **kwargs):
        ...
        if self.ctrl.num_epochs > 1:
            if isinstance(self.env.env, Quadrotor):
                ...
                make_plots(test_runs=test_runs, train_runs=train_runs, dir=self.ctrl.output_dir)
        print("Training done.")
