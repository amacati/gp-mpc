...
class LinearModel:
    ...
    def __init__(self, input_data, target_data, feature: cs.Function):
        ...
        self.feature_data = np.array(
            [self.feature(self.input_data[i]) for i in range(self.num_data)]
        ).squeeze()
    ...
    def predict(self, input_data):
        feature_data = np.array(
            [self.feature(input_data[i]) for i in range(input_data.shape[0])]
        ).squeeze()
