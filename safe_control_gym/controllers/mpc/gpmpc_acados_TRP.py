...
class GPMPC_ACADOS_TRP:
    ...
    def select_action_with_gp(self, obs):
        ...
        time_after = time.time()
        print(
            f'gpmpc acados sol time: {time_after - time_before:.3f}; sol status {status}; nlp iter {self.acados_ocp_solver.get_stats("sqp_iter")}; qp iter {self.acados_ocp_solver.get_stats("qp_iter")}'
        )
        if time_after - time_before > 1 / 60:
            print(
                colored(
                    f"========= Warning: GPMPC ACADOS took {time_after - time_before:.3f} seconds =========",
                    "yellow",
                )
            )
    ...
    def reset(self):
        ...
        if self.gaussian_process is not None:
            ...
            self.acados_ocp_solver = AcadosOcpSolver(
                self.ocp, self.output_dir + f"/gpmpc_acados_ocp_solver.json"
            )
    ...
    def plot_gp_TP(self, train_inputs, train_targets, title=None, output_dir=None):
        ...
        residual_P = (
            self.P_res_func(
                train_inputs[:, 1].reshape(-1, 1),
                train_inputs[:, 2].reshape(-1, 1),
                train_inputs[:, 3].reshape(-1, 1),
            )
            .full()
            .flatten()
        )
        num_within_2std_T = np.sum(
            (train_targets[:, 0] > lower_T) & (train_targets[:, 0] < upper_T)
        )
        percentage_within_2std_T = num_within_2std_T / num_data * 100
        num_within_2std_P = np.sum(
            (train_targets[:, 1] > lower_P) & (train_targets[:, 1] < upper_P)
        )
    ...
    def learn(self, env=None):
        ...
        for epoch in range(1, self.num_epochs):
            ...
            episode_length = train_runs[epoch - 1][self.num_train_episodes_per_epoch - 1][0]["obs"][
                0
            ].shape[0]
            if True:
