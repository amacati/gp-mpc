...
class GaussianProcess:
    ...
    def _compute_GP_covariances(self, train_x):
        ...
        print(
            colored(
                f"Condition number of the covariance matrix: {torch.linalg.cond(self.model.K_plus_noise)}",
                "green",
            )
        )
    ...
    def train(
        ...
        print(
            colored(
                f"final lengthscale: {self.model.covar_module.base_kernel.lengthscale}", "green"
            )
        )
    ...
    def plot_trained_gp(
        ...
        print(
            f"Percentage of test points within 2 std for dim {output_label}: {percentage_within_2std:.2f}%"
        )
        plt.fill_between(
