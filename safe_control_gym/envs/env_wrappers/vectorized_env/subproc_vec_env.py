...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context="spawn", n_workers=1):
        ...
        self.remotes, self.work_remotes = zip(*[ctx.Pipe() for _ in range(self.n_workers)])
        self.ps = [
    ...
    def env_method(self, method_name, method_args=None, method_kwargs=None, indices=None):
        ...
        for remote, env_indices, method_arg_split, method_kwarg_split in zip(
            ...
            remote.send(
                ("env_method", (env_indices, method_name, method_arg_split, method_kwarg_split))
            )
        return _flatten_list([remote.recv() for remote in target_remotes])
    ...
    try:
        while True:
            ...
            if cmd == "step":
                ...
                target_envs = [envs[idx] for idx in env_indices]
                remote.send(
                    [setattr(env, attr_name, value) for env, value in zip(target_envs, values)]
                )
            elif cmd == "env_method":
