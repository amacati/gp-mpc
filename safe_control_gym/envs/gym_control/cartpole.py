...
class CartPole(BenchmarkEnv):
    ...
    def step(self, action):
        ...
        self.state = np.hstack(
            (
                p.getJointState(self.CARTPOLE_ID, jointIndex=0, physicsClientId=self.PYB_CLIENT)[
                    0:2
                ],
                p.getJointState(self.CARTPOLE_ID, jointIndex=1, physicsClientId=self.PYB_CLIENT)[
                    0:2
                ],
            )
        )
    ...
    def _setup_symbolic(self, prior_prop={}, **kwargs):
        ...
        X_dot = cs.vertcat(
            x_dot, temp_factor - ml * theta_dot_dot * cs.cos(theta) / Mm, theta_dot, theta_dot_dot
        )
        # Observation.
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):
            ...
            if tab_force is not None:
                ...
                if self.GUI:
                    center = np.asarray(
                        p.getLinkState(
                            self.CARTPOLE_ID, linkIndex=1, physicsClientId=self.PYB_CLIENT
                        )[0]
                    )
