...
class BaseAviary(BenchmarkEnv):
    ...
    def _start_video_recording(self):
        ...
        if self.RECORD and not self.GUI:
            ...
            self.IMG_PATH = os.path.join(
                self.output_dir,
                "quadrotor_videos/video-{}/".format(datetime.now().strftime("%m.%d.%Y_%H.%M.%S")),
            )
    ...
    def setup_rk4_dynamics_expression(self):
        ...
        oVdot_cg_o = Rob @ cs.vertcat(0, 0, f1 + f2 + f3 + f4) / self.MASS - cs.vertcat(
        ...
        self.fd_func = cs.integrator(
            "fd", "rk", {"x": X, "p": U, "ode": X_dot}, {"tf": self.PYB_TIMESTEP}
        )
    ...
    def setup_dynamics_2d_expression(self):
        ...
        # We are using the velocity of the base wrt to the world frame expressed in the world frame.
        # Note that the reference expresses this in the body frame.
        pos_ddot = Rob @ cs.vertcat(0, 0, f1 + f2 + f3 + f4) / self.MASS - cs.vertcat(
            0, 0, self.GRAVITY_ACC
        )
        pos_dot = cs.vertcat(x_dot, y_dot, z_dot)
        Mb = cs.vertcat(
    ...
    def _dynamics_si(self, action, nth_drone, disturbance_force=None):
        ...
        self.ang_v[nth_drone, :] = get_angularvelocity_rpy(
            self.rpy[nth_drone, :], self.rpy_rates[nth_drone, :]
        )
    ...
    def setup_dynamics_si_3d_expression(self):
        ...
        X_dot = cs.vertcat(
            ...
            * (cs.cos(phi) * cs.sin(theta) * cs.sin(psi) - cs.sin(phi) * cs.cos(psi)),
            z_dot,
            (params_acc[0] * T + params_acc[1]) * cs.cos(phi) * cs.cos(theta)
            - g
            + d[1] / self.MASS,
            phi_dot,
            theta_dot,
