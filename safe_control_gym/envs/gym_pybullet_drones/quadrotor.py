...
class Quadrotor(BaseAviary):
        ...
        **kwargs,
        ...
        if info_mse_metric_state_weight is None:
            ...
            # elif self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE:
                ...
                self.info_mse_metric_state_weight = np.array(
                    [1, 0, 1, 0, 1, 0, 0, 0, 0, 0], ndmin=1, dtype=float
                )
            else:
            ...
            if (
                ...
                or (
                    self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE
                ...
                or (
                    ...
                    and len(info_mse_metric_state_weight) == 5
                )
                or (
                    self.QUAD_TYPE == QuadType.THREE_D_ATTITUDE
                    and len(info_mse_metric_state_weight) == 12
                )
                or (
                    self.QUAD_TYPE == QuadType.THREE_D_ATTITUDE_10
                    and len(info_mse_metric_state_weight) == 10
                )
            ):
                self.info_mse_metric_state_weight = np.array(
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            or self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE
            or self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE_5S
        ...
        if inertial_prop is None:
            ...
            self.MASS, self.J[1, 1] = inertial_prop
        elif self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE_BODY and np.array(inertial_prop).shape == (
            2,
        ):
            self.MASS, self.J[1, 1] = inertial_prop
    ...
    def set_goals(self):
        ...
        if self.TASK == Task.STABILIZATION:
            ...
            if use_ilqr_ref:
                assert (
                    self.QUAD_TYPE in [QuadType.TWO_D_ATTITUDE, QuadType.THREE_D_ATTITUDE]
                ...
                # if
                POS_REF = np.array(
                    [traj_data["obs"][:, 0], 0 * traj_data["obs"][:, 0], traj_data["obs"][:, 2]]
                ).T
                VEL_REF = np.array(
                    [traj_data["obs"][:, 1], 0 * traj_data["obs"][:, 1], traj_data["obs"][:, 3]]
                ).T
                ...
                if keys in self.disturbances:
                    if keys == "downwash":
                        ...
                        if isinstance(self.disturbances[keys].disturbances[0].std, list):
                            self.disturbances[keys].disturbances[0].std = list(
                                np.array(self.disturbances[keys].disturbances[0].std)
                                * np.random.choice(values[0]["scale"])
                            )
                        elif isinstance(self.disturbances[keys].disturbances[0].std, float):
                            self.disturbances[keys].disturbances[0].std = self.disturbances[
                                keys
                            ].disturbances[0].std * np.random.choice(values[0]["scale"])
    ...
    def step(self, action):
        ...
        if "downwash" in self.disturbances:
            assert self.QUAD_TYPE in [
                QuadType.TWO_D_ATTITUDE,
                QuadType.TWO_D_ATTITUDE_5S,
                QuadType.TWO_D_ATTITUDE_BODY,
                QuadType.THREE_D_ATTITUDE,
                QuadType.THREE_D_ATTITUDE_10,
            ], "[ERROR] in Quadrotor.step(), downwash model is only available for identified model."
            ...
            if self.QUAD_TYPE not in [QuadType.TWO_D_ATTITUDE]:
                raise ValueError(
                    "[ERROR] in Quadrotor.step(), downwash force is only available for 2D attitude model."
                )
            if self.QUAD_TYPE in [
                QuadType.TWO_D_ATTITUDE,
                QuadType.TWO_D_ATTITUDE_BODY,
    ...
    def _setup_symbolic(self, prior_prop={}, **kwargs):
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            X_dot = cs.vertcat(
                ...
                theta_dot,
                self.alpha_1 * (theta + self.pitch_bias)
                + self.alpha_2 * theta_dot
                + self.alpha_3 * P,
            )
            # Define observation.
            Y = cs.vertcat(x, x_dot, z, z_dot, theta, theta_dot)
            ...
            parameterized_X_dot = cs.vertcat(
                x_dot,
                (lr_param[0] * mp[0] * T + lr_param[1] * mp[1]) * cs.sin(theta)
                + lr_param[2] * mp[2],
                z_dot,
                (lr_param[0] * mp[0] * T + lr_param[1] * mp[1]) * cs.cos(theta) - g,
                theta_dot,
                -lr_param[3] * mp[3] * theta
                - lr_param[4] * mp[4] * theta_dot
                + lr_param[5] * mp[5] * P,
            )
            ...
            # Define dynamics equations.
            X_dot = cs.vertcat(
                vx * cs.cos(theta) - vz * cs.sin(theta),
            ...
            X = cs.vertcat(
                x, x_dot, y, y_dot, z, z_dot, phi, theta, psi, phi_dot, theta_dot, psi_dot
            )
            # Define input collective thrust and theta.
            T = cs.MX.sym("T_c")  # normalized thrust [N]
            ...
            X_dot = cs.vertcat(
                ...
                * (cs.cos(phi) * cs.sin(theta) * cs.sin(psi) - cs.sin(phi) * cs.cos(psi)),
                ...
                params_pitch_rate[0] * theta
                + params_pitch_rate[1] * theta_dot
                + params_pitch_rate[2] * P,
    ...
    def _set_action_space(self):
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            self.ACTION_UNITS = (
                ["N", "rad", "rad"] if not self.NORMALIZED_RL_ACTION_SPACE else ["-", "-", "-"]
            )
        ...
        if self.QUAD_TYPE in [
            ...
            self.physical_action_bounds = (
                np.array(
                    [np.full(1, a_low, np.float32), np.full(1, -max_pitch_rad, np.float32)]
                ).flatten(),
                np.array(
                    [np.full(1, a_high, np.float32), np.full(1, max_pitch_rad, np.float32)]
                ).flatten(),
            )
        elif self.QUAD_TYPE == QuadType.THREE_D_ATTITUDE:
            n_mot = 4  # due to collective thrust
            ...
            self.physical_action_bounds = (
                np.array(
                    [
                        ...
                        np.full(1, -max_yaw_rad, np.float32),
                    ]
                ).flatten(),
                np.array(
                    [
    ...
    def _set_observation_space(self):
        ...
        # elif self.QUAD_TYPE == QuadType.TWO_D or self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE:
            ...
            self.STATE_UNITS = ["m", "m/s", "m", "m/s", "m", "m/s", "rad", "rad", "rad/s", "rad/s"]
    ...
    def _preprocess_control(self, action):
        ...
        if "action" in self.disturbances:
            self.current_physical_action = self.disturbances["action"].apply(
        ...
        self.current_clipped_action = np.clip(
            self.current_noisy_physical_action, self.action_space.low, self.action_space.high
        )
        ...
        if self.QUAD_TYPE in [
            ...
            if self.PHYSICS == Physics.DYN_2D:
                ...
                _, quat = p.getBasePositionAndOrientation(
                    self.DRONE_IDS[0], physicsClientId=self.PYB_CLIENT
                )
            thrust_action = self.attitude_control._dslPIDAttitudeControl(
                collective_thrust / 4, quat, np.array([0, pitch, 0])
            )
            # input thrust is in Newton
            thrust = np.array(
                [thrust_action[0] + thrust_action[3], thrust_action[1] + thrust_action[2]]
            )
            thrust = np.clip(
                thrust,
                np.full(2, self.physical_action_bounds[0][0] / 2),
                np.full(2, self.physical_action_bounds[1][0] / 2),
            )
            pitch = np.clip(
                pitch, self.physical_action_bounds[0][1], self.physical_action_bounds[1][1]
            )
    ...
    def _get_observation(self):
        ...
        if self.QUAD_TYPE == QuadType.ONE_D:
            ...
            # {x, x_dot, z, z_dot, theta, theta_dot}.
            self.state = np.hstack([pos[0], vel[0], pos[2], vel[2], rpy[1], rpy_rate[1]]).reshape(
                (6,)
            )
        elif self.QUAD_TYPE == QuadType.TWO_D_ATTITUDE_BODY:
            ...
            # {x, x_dot, z, z_dot, theta, theta_dot}.
            self.state = np.hstack([pos[0], vel[0], pos[2], vel[2], rpy[1]]).reshape((5,))
        elif self.QUAD_TYPE == QuadType.THREE_D:
