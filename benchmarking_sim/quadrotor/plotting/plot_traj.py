...
ax[0].plot(X_GOAL[:, 0], X_GOAL[:, 2], color="k", linestyle=":", label="Figure-8")
ax[0].plot(
    mpc_traj_data["ref"][:, 0],
    mpc_traj_data["ref"][:, 2],
    color="red",
...
ax[0].plot(mpc_traj_data["obs"][:, 0], mpc_traj_data["obs"][:, 2], color=colors[0], label="MPC")
ax[0].plot(ilqr_traj_data["obs"][:, 0], ilqr_traj_data["obs"][:, 2], color=colors[1], label="iLQR")
ax[0].set_xlabel("x [m]")
ax[0].set_ylabel("z [m]")
ax[0].legend()
ax[0].set_title(
    f'Trajectory path x-z \n MPC RMSE {mpc_traj_data["rmse"]:.3f} [m], iLQR RMSE {ilqr_traj_data["rmse"]:.3f} [m]'
)
...
fig.tight_layout()
fig.savefig(
    os.path.join(script_path, f"{ref_type}_ref_traj_{episode_len}.png"), bbox_inches="tight"
)

print(
    f'length sec: {episode_len}, MPC RMSE: {mpc_traj_data["rmse"]:.3f}, iLQR RMSE: {ilqr_traj_data["rmse"]:.3f}'
)
# # plot a table
