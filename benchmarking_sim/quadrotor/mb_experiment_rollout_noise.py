...
def run(gui=False, n_episodes=1, n_steps=None, save_data=True, seed=2):
    ...
    assert os.path.exists(f'./config_overrides/{SYS}_{TASK}{EXTRA_TASK}.yaml'), f'./config_overrides/{SYS}_{TASK}{EXTRA_TASK}.yaml does not exist'
    ...
    if SAFETY_FILTER is None:
        sys.argv[1:] = ['--algo', ALGO,
                        '--task', agent,
                        '--overrides',
                            f'./config_overrides/{SYS}_{TASK}{EXTRA_TASK}.yaml',
    ...
    if ALGO in ['gpmpc_acados', 'gp_mpc']:
        ...
        config.output_dir = os.path.join(config.output_dir, PRIOR + '_' + repr(num_data_max) + '_noise')
    else:
    ...
    noise_factor_end = 100
    noise_factor_step = 2
    default_noise_std = config.task_config.disturbances.observation[0]['std']
    for i in range(noise_factor_start, noise_factor_end, noise_factor_step):
        # amplify the observation noise std with a factor 
        noise_factor = i
        print(colored(f'Original observation noise std: {default_noise_std}', 'green'))
        config.task_config.disturbances.observation[0]['std'] = [noise_factor * default_noise_std[i] for i in range(len(default_noise_std))]
        print(colored(f'Amplified observation noise std: {config.task_config.disturbances.observation[0]["std"]}', 'green'))
        config.output_dir = output_dir + f'/noise_{noise_factor}'
        mkdirs(config.output_dir)
        ...
        total_traj_length = np.floor(config.task_config.episode_len_sec * config.task_config.ctrl_freq)
        metrics['early_stop'] = True if trajs_data['obs'][0].shape[0] < total_traj_length else False
        ...
        if save_data:
            ...
            with open(f'./{config.output_dir}/metrics.txt', 'w') as f:
                for key, value in metrics.items():
                    f.write(f'{key}: {value}\n')
                print(f'Metrics saved to ./{config.output_dir}/metrics_{noise_factor}.txt')

        print('FINAL METRICS - ' + ', '.join([f'{key}: {value}' for key, value in metrics.items()]))
        env_func = None
        ctrl = None
        static_env = None
        static_train_env = None
        random_env = None
        experiment = None
